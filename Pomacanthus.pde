...
final float s = 10; //scaling factor (one of Turing model parameters). default value : 6
final float r = 6;//reaction factor (one of Turing model parameters). default value : 30
float g = 0.00;//growth rate. initial default value : 0.00
final float g1 = 0.05;//final growth rate. default value : 0.05
...
int t = 0;
final float sigma = 0.1; //écart type loi normale stripes
final float mu = largeur/2; // position central stripe
final float r2 = 0; //Ictydomys : 0.1
int tpicture = 0;
...
void draw() {
  ...
  d = constrain(d,0.275,1); //implementing indirect exponential growth
  t = t+1;
  tpicture = tpicture+1;
  //print(d);
  print(t + "  ");
  ...
  for (int i = 0; i < hauteur-1; i++) {
    for (int j = 0; j < largeur-1; j++) {
      float pixelcolora = -constrain(100*(a0[i][j]-3)*2.5/4.4,0,100)+100; // for Pomacanthus
      //float pixelcolorb = b0[i][j]-3.5; // for Ictydomys
      int pos = j + i * largeur;
      ...
      if (pixelcolora >= 50) {
        pixels[pos] = color(240, 100, 100-2*(pixelcolora-50));//
      }
      if (pixelcolora < 50) {
        pixels[pos] = color(240, 2*pixelcolora, 100);//
      }
      pixels[pos] = color(240, pixelcolora, 100-pixelcolora);//
  ...
  for (int k = 0; k < largeur-1; k++) {
    ...
    int pos3 = (500-pos2)*largeur + k;
    pixels[pos3] = color (120,100,50);
  }
  
   for (int l = 0; l < largeur-1; l++) {
     pixels[275*250+l] = color (120,0,0);
     pixels[458*250+l] = color (120,0,0);
   }
  
  updatePixels();
  if (t == 1) {
    save(str(r) + "Pomacanthus g=0.05 t=1.png");
  }
  if (tpicture == 250) {
    if (t <= 100001) {
    save("Pomacanthus g=0.05 t = " + str(t) + ".png");
    tpicture = 0;
    }
}
    if (t > 20000) {
