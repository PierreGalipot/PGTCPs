...
final float s = 10 ; //facteur de scaling (paramètre du modèle de Turing). default value : 3
final float r = 8;//facteur de reaction (paramètre du modèle de Turing). default value : 20
float g = 0.00;//growth rate. initial default value : 0.00
final float g1 = 0.075;//final growth rate. default value : 0.05
...
void draw() {
  ...
  for (int i = 0; i < hauteur-1; i++) {
    for (int j = 0; j < largeur-1; j++) {
      ...
      float pixelcolorb = b0[i][j]-2.5; // for Ictydomys
      int pos = j + i * largeur;
  ...
  updatePixels();
  if (t == 1) {
    save(str(r) + " Tetraodon t=1.png");
  }
  if (tpicture == 250) {
    if (t <= 100001) {
    save("Tetraodon g=0.725 t = " + str(t) + ".png");
    tpicture = 0;
    }
  }
